...
export default function DriverDashboard() {
  ...
  useEffect(()=>{
    getDocs(collection(db,"routes")).then(snap=>{
      if(!snap.empty){const r=snap.docs.map(d=>({id:d.id,...d.data()}));setRoutes(r);setSelectedRouteId(r[0].id);}
      setLoading(false);
  ...
  function formatTime(s){const h=Math.floor(s/3600),m=Math.floor((s%3600)/60),sec=s%60;return h>0?`${h}:${String(m).padStart(2,"0")}:${String(sec).padStart(2,"0")}`:`${String(m).padStart(2,"0")}:${String(sec).padStart(2,"0")}`;}
  function formatDuration(ms){const s=Math.floor(ms/1000),h=Math.floor(s/3600),m=Math.floor((s%3600)/60);return h>0?`${h}h ${m}m`:`${m}m`;}
  ...
  async function startTracking(){
    ...
    watchIdRef.current=navigator.geolocation.watchPosition(pos=>{
      ...
      const now=Date.now();
      setMyLocation({lat,lng}); setSpeed(kmh); setHeading(hdg||0); setAccuracy(acc?Math.round(acc):null);
    ...
    <div style={{minHeight:"100vh",background:t.bg,fontFamily:"'DM Sans',sans-serif",color:t.text,transition:"background 0.3s,color 0.3s"}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 16px",borderBottom:`1px solid ${t.border}`,position:"sticky",top:0,background:t.headerBg,backdropFilter:"blur(12px)",zIndex:20}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <button onClick={toggle} title={dark?"Light mode":"Dark mode"} style={{width:36,height:36,borderRadius:10,border:`1px solid ${t.border}`,background:t.bgCard,cursor:"pointer",fontSize:18,display:"flex",alignItems:"center",justifyContent:"center"}}>
            {dark?"☀️":"🌙"}
          </button>
              ...
              <>
                ...
                <div style={{background:t.bgCard,border:`1px solid ${t.border}`,borderRadius:16,overflow:"hidden",marginBottom:14}}>
                  ...
                  <div style={{display:"flex",padding:"16px"}}>
                    {[
                      ...
                      [tracking?"Live":"Off","GPS",tracking?"#4ADE80":t.textHint],
                  ...
                  {tracking&&accuracy!==null&&(
                    <div style={{padding:"0 16px 14px",display:"flex",alignItems:"center",gap:8}}>
                      <div style={{flex:1,height:3,background:t.border,borderRadius:2,overflow:"hidden"}}>
                        <div style={{height:"100%",width:`${Math.max(0,100-accuracy)}%`,background:accuracy<10?"#4ADE80":accuracy<30?"#facc15":"#F87171",transition:"width 0.5s",borderRadius:2}}/>
                      </div>
                ...
                {tracking&&(
                  <div style={{background:dark?"#0A1A0D":"#ECFDF5",border:`1px solid ${dark?"#1A3D22":"#6EE7B7"}`,borderRadius:10,padding:"10px 14px",marginBottom:14,display:"flex",alignItems:"center",gap:8}}>
                    ...
                    <span style={{fontSize:12,color:dark?"#4ADE80":"#065F46",fontWeight:500}}>Broadcasting live · Firebase every 2s</span>
                  </div>
                )}
        ...
        {tab==="trips"&&(
          <>
            ...
            {trips.length===0
              ...
              :trips.map(trip=>(
                <div key={trip.id} style={{background:t.bgCard,border:`1px solid ${t.border}`,borderRadius:14,padding:"14px 16px",marginBottom:10}}>
