...
export default function DriverDashboard() {
  ...
  // Pause Firebase updates when app is backgrounded
  useEffect(() => {
    const handleVisibility = () => {
      if (document.hidden && watchIdRef.current) {
        // Keep GPS but slow down Firebase writes
  ...
  async function startTracking() {
    ...
    watchIdRef.current = navigator.geolocation.watchPosition(
      pos => {
        ...
        // Throttle map update — every 3 seconds
        ...
        // Throttle Firebase update — every 5 seconds
  ...
  const S = {
    ...
    label: { fontSize: 10, color: "#2A2A2A", fontWeight: 600, textTransform: "uppercase", letterSpacing: "1px", marginBottom: 8 },
    routeBtn: (sel) => ({ flex: 1, padding: "10px 4px", border: `1px solid ${sel ? "#FF5A1F" : "#141414"}`, borderRadius: 10, background: sel ? "#150D09" : "#0D0D0D", color: sel ? "#FF5A1F" : "#444", fontSize: 11, fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }),
      ...
      <div style={S.body}>
          ...
          <>
            ...
            <div style={S.card}>
              <div style={{ textAlign: "center", padding: "28px 16px", borderBottom: "1px solid #111" }}>
                <div style={{ fontSize: 10, color: "#2A2A2A", textTransform: "uppercase", letterSpacing: "1px", marginBottom: 8 }}>
              ...
              <div style={{ display: "flex", padding: "14px 16px" }}>
                {[
                  ...
                  <div key={i} style={{ flex: 1, textAlign: "center" }}>
                    ...
                    <div style={{ fontSize: 10, color: "#2A2A2A", marginTop: 2 }}>{label}</div>
            ...
            {myLocation && (
              <div style={{ background: "#0D0D0D", border: "1px solid #141414", borderRadius: 12, padding: "12px 16px" }}>
                <p style={{ fontSize: 10, color: "#2A2A2A", textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 4 }}>GPS Location</p>
        ...
        {tab === "trips" && (
          <>
            ...
            {trips.length === 0
              ? <div style={{ textAlign: "center", padding: "40px 0", color: "#1A1A1A", fontSize: 14 }}>No trips yet</div>
              : trips.map(trip => (
                <div key={trip.id} style={S.tripCard}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                    <div>
                      <div style={{ fontSize: 14, fontWeight: 600, color: "#bbb" }}>{trip.routeName}</div>
                      <div style={{ fontSize: 11, color: "#2A2A2A", marginTop: 2 }}>{new Date(trip.startTime).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</div>
                    </div>
                  ...
                  <div style={{ display: "flex", gap: 16 }}>
                    <div>
                      <div style={{ fontSize: 10, color: "#2A2A2A", textTransform: "uppercase", letterSpacing: "0.8px" }}>Start</div>
                      <div style={{ fontSize: 13, color: "#777", marginTop: 2 }}>{new Date(trip.startTime).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</div>
                    </div>
                    {trip.endTime && (
                      <>
                        <div style={{ color: "#1A1A1A", alignSelf: "flex-end" }}>→</div>
                        <div>
                          <div style={{ fontSize: 10, color: "#2A2A2A", textTransform: "uppercase", letterSpacing: "0.8px" }}>End</div>
                          <div style={{ fontSize: 13, color: "#777", marginTop: 2 }}>{new Date(trip.endTime).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</div>
                        </div>
                        <div>
                          <div style={{ fontSize: 10, color: "#2A2A2A", textTransform: "uppercase", letterSpacing: "0.8px" }}>Duration</div>
