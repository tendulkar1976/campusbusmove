...
export default function AdminDashboard() {
  ...
  const S = {
    ...
    tabBtn: (a) => ({ padding: "12px 16px", border: "none", background: "none", cursor: "pointer", fontSize: 13, fontWeight: 500, color: a ? "#FF5A1F" : "#FF5A1F", borderBottom: a ? "2px solid #FF5A1F" : "2px solid transparent", fontFamily: "'DM Sans', sans-serif", transition: "color 0.2s" }),
    ...
    statLabel: { fontSize: 10, color: "#FF5A1F", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.8px" },
    ...
    cardLabel: { fontSize: 10, color: "#FF5A1F", fontWeight: 600, textTransform: "uppercase", letterSpacing: "1px" },
    ...
    routePill: (a) => ({ fontSize: 10, padding: "3px 8px", borderRadius: 20, background: a ? "#0A1A0D" : "#0D0D0D", color: a ? "#4ADE80" : "#FF5A1F", border: `1px solid ${a ? "#1A3D22" : "#141414"}` }),
    rolePill: (r) => ({ fontSize: 11, padding: "3px 8px", borderRadius: 6, background: r === "admin" ? "#1A0F2A" : r === "driver" ? "#0A1020" : "#111", color: r === "admin" ? "#A78BFA" : r === "driver" ? "#60A5FA" : "#FF5A1F", border: `1px solid ${r === "admin" ? "#2D1F55" : r === "driver" ? "#1A2D55" : "#141414"}` }),
                ...
                return (
                  <div key={pr.id} style={S.row}>
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: "#FF5A1F", marginTop: 2 }}>{displayRoute.label}</div>
                      {active && driverName && (
                        <div style={{ fontSize: 11, color: "#FF5A1F", marginTop: 3 }}>🚌 {driverName} · {speed} km/h</div>
                      )}
                      {!active && (
                        <div style={{ fontSize: 11, color: "#FF5A1F", marginTop: 3 }}>No driver active</div>
                      )}
                    </div>
                    ...
                    {editingRoute?.id === pr.id ? (
                      <div style={{ width: "100%" }}>
                        ...
                        <div style={{ display: "flex", gap: 8 }}>
                          ...
                          <button onClick={() => setEditingRoute(null)} style={{ flex: 1, background: "none", border: "1px solid #1A1A1A", borderRadius: 8, padding: "8px 0", color: "#FF5A1F", fontSize: 12, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
                    ...
                    {editingRoute?.id === route.id ? (
                      ...
                      <div style={{ display: "flex", width: "100%", alignItems: "center", justifyContent: "space-between" }}>
                        <div>
                          ...
                          <div style={{ fontSize: 11, color: "#FF5A1F", marginTop: 2 }}>{route.label}</div>
                        </div>
                        <div style={{ display: "flex", gap: 8 }}>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
              {[["Total", users.length, "#fff"], ["Students", studentCount, "#60A5FA"], ["Drivers", driverCount, "#A78BFA"], ["Blocked", blockedCount, "#F87171"]].map(([l, v, c]) => (
                <div key={l} style={{ flex: 1, background: "#0D0D0D", border: "1px solid #141414", borderRadius: 12, padding: "12px 8px", textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: 10, color: "#FF5A1F", marginTop: 2, textTransform: "uppercase", letterSpacing: "0.6px" }}>{l}</div>
                </div>
              ))}
            </div>
                ...
                : users.map(u => (
                  <div key={u.id} style={{ ...S.row, flexDirection: "column", alignItems: "flex-start", gap: 10, background: u.blocked ? "#0D0808" : "transparent" }}>
                    <div style={{ display: "flex", width: "100%", alignItems: "center", justifyContent: "space-between" }}>
                      <div>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <div style={{ fontSize: 13, color: u.blocked ? "#FF5A1F" : "#bbb", fontWeight: 500, textDecoration: u.blocked ? "line-through" : "none" }}>{u.name || "—"}</div>
